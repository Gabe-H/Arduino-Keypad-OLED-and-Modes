...
<br>
This keypad program brings a minimal, change-on-the-fly macro pad.<br>
<br>
I came up with the idea after the first time I keypad with modes, but I never managed to fill every button with practical macros, and was constantly switching from mode to mode. This keypad allows you to have multiple modes on a single pad, separated by each row. A rotary encoder will (by default) change volume while idling, but press it and you can quickly easily change each row's mode.<br>
...
- `EEPROM_INIT` - Uncomment to reset the EEPROM that will be used later. Sometimes needed if the board starts in a bad mode.
...
- `buttonType` - Use K/C to identify each bind as a Keyboard or Consumer button (Most ASCII and F keys will be Keyboard (K), most media/function keys will be Consumer (C)) or ignore a key with `UNUSED`
